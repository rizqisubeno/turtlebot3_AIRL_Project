...
class Agent(gym.Env, Supervisor, RobotSubscriber, AgentPublisher):

    def __init__(self, 
                 name_exp,
                 agent_settings,
                 scene_configuration,
                 fixed_steps: Optional[bool]=True,
                 logging_reward : Optional[bool]=False,
        ...
        self.writer.add_text(
            "Agent_Params",
            "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in vars(self.agent_settings).items()])),
        )
    ...
    def calculate_reward(self,
        ...
        if distanceToTarget <= self.agent_settings.goal_dist:
            ...
            info["target"]=False
            return (-distance) + (r_safety) + ((action[0]+1.0) / 3.0) + (-abs(action[1]) / 3.0), info
    ...
    def step(self, action):
        ...
        if(self.turn_off_act):
            ...
            action = [-1.000, 0.000]    #-1.000 for minimum linear vel & 0.000 for not turn 
        ...
        state = np.append(state, np.array([self.robo_subscriber.states.distance_length,
                                  self.robo_subscriber.states.angular_length]))
        # print(f"{state=}")
        collision, min_laser = self.observe_collision(state[:self.agent_settings.lidar_for_state])
        # if(collision):
        #     logger("warning", "Py", f"{collision=}")
        reward, info = self.calculate_reward(state[self.agent_settings.lidar_for_state], 
                                       min_laser,
                                       action)
        if(self.logging_reward):
        ...
        target = [True if (key=="target" and value==True) else False for key, value in info.items()][0]
        collision = [True if (key=="collision" and value==True) else False for key, value in info.items()][0]
        ...
        if(self.toggle_first_print and (target or collision)):
            ...
            if(target):
                ...
                logger("hidden", "Py", f"Reach Goal!Scene={self.scenario_idx+1}, counter={self.success_counter}")
        ...
        if ((done or truncated) and self.logging_reward):
            logger("info", head_name, f"change episode scene: {self.scenario_idx+1}, cause: {'target' if target else ('collision' if collision else 'truncated')}")
            reward_total = np.sum(np.asarray([r*(self.agent_settings.gamma**q) for q, r in enumerate(self.reward_list)]))
            info['episode'] = {'r': reward_total,
                               'l': self.idx}
            #adding to tensorboard
            self.writer.add_scalar(f"Reward/scene_{self.scenario_idx+1}", reward_total, self.step_idx)
            self.reward_list = []
    ...
    def reset(self, seed: Optional[int]=1):
        ...
        while ("start" not in self.agent_publisher.subscriptions or 
               "step" not in self.agent_publisher.subscriptions):
            self.agent_publisher.receiveSubscriptions()
        if(self.success_counter >=self.scene_cfg.change_scene_every_goal_reach and self.eval==False):
            self.success_counter = 0
            self.scenario_idx += 1
        ...
        start_msg.max_steps = self.scene_cfg.max_steps if (self.scene_cfg.max_steps>=1e5) else 1024
        ...
        state = self.robo_subscriber.states.lidar_data[:self.agent_settings.lidar_for_state]
        state = np.asarray(state)
        state = np.append(state, [self.robo_subscriber.states.distance_length,
                                  self.robo_subscriber.states.angular_length])
        # print(f"{state=}")
        info = {}
        self.toggle_success_counter = True
